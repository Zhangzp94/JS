...
- clearRect(x,y,w,h) 擦除矩形区域--exp: ctx.clearRect( 0,0,ctx.canvas.width,ctx.canvas.height)
...
- 平移 移动画布的原点
    ...
    + 画布中绘制的图形都是以坐标为原点，translate可以移动原点的坐标
- 缩放
    + scale(x,y) 参数表示宽高的缩放比例--x y轴的缩放比例，scale( 0.5 ,1 )
- 旋转
    + rotate(angle) 参数表示旋转角度
    + exp:  rotate( Math.PI/2 )
